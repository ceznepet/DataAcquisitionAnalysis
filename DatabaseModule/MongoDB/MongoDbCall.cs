...
namespace DatabaseModule.MongoDB
{
    ...
    {
        public static void LoadDataAndSave(string databaseLocation, string database, string document, string profinet, string folder, string fileName)
        {
            var mongoLoader = new MongoLoader(databaseLocation, database, document, profinet, folder, fileName);
